...
const chapterTwo = () => ({
  ...
  partB: {
    ...
    paragraph3: `Well, fuck if ${truck.pilot.firstName} wasn't persuaded to just pay a little visit to the ${mainShip.name}. "${truck.assistant.name}, I'm going back to sleep. Change course for ${mainShip.name} and wake me when we arrive." commanded ${truck.pilot.firstName}. "Of course, sir. by the way, have you heard the one about the clockmaker and the farmer's daughter?"
      "Haha, yea she really wound him up! goodnight, ${truck.assistant.name}"
      "Goodnight, sir"`
  },
  partC: {
    paragraph1: `"Oh for goodness sake. I've had quite enough of this. ${truck.pilot.firstName}, WAKE UP!" ${truck.assistant.name} raised the volume of its vocal processors so immensely for that last part that ${truck.pilot.firstName} smacked her head agaist the ceiling of the cockpit and farted as she came bolt upright. This was not the best way to meet ${mainShip.firstOfficer.fullName}. "My god, what do you truckers eat?" queried ${mainShip.firstOfficer.title} ${mainShip.firstOfficer.lastName}. "Umm" ${truck.pilot.firstName} was helpless for a moment. "Look, let's just step out of the ${truck.name}. So we can speak about matters at hand."
  ...
  partD: {
    paragraph1: `"Let's see here, we're holding ${converter.toWords(numOfPies)} Turkey Pot-Pies for a ${truck.pilot.lastName}, ${truck.pilot.firstName} handle: ${truck.pilot.handle} of the ${truck.company} to be transported to ${desinationShip.name}. Do you speak on helaf of this ${truck.pilot.handle}?" ${mainShip.cargoMaster.fullName} Looked vacant-eyed at ${truck.pilot.firstName} over the desk of the cargomasters office.
  ...
  "No problem. Just get one from the ${mainShip.name} office of secure identification. It's just next door. Shouldn't take but a moment. I will close my desk and cacth up on some paper work while you do that. You wont have to wait in line when you return." ${truck.pilot.firstName} looked around the room. It was empty aside from ${mainShip.cargoMaster.title} ${mainShip.cargoMaster.lastName}. `,
    paragraph2: `${truck.pilot.firstName} made her way to the room next door which looked almost exactly the same, except that behind the desk was a different vacant-eyed council employee and a little sign on the desk read "${mainShip.idOfficer.fullName}". 
